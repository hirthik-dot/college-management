  ...
  useEffect(() => {
    const fetchArrears = async () => {
      try {
        ...
        // collect arrears across documents
        const all = (data.documents || []).flatMap(d => (d.arrears || []).map(a => ({
          ...a,
          sem: d.sem,
          fileUrl: d.fileUrl
        })));
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: "1.1rem", marginTop: "6px" }}>View and manage all your backlog subjects in one place.</div>
        </div>
      </div>
      ...
      <div style={{ width: "95%", maxWidth: "1300px", padding: "0", margin: "34px auto 0 auto", display: "flex", gap: "38px", alignItems: "flex-start" }}>
        <div style={{
          ...
          {loading ? <div>Loading...</div> : arrears.length === 0 ? (
            <div style={{ color: "#22c55e", fontWeight: 700 }}>No arrears found! 🎉</div>
          ) : arrears.map((a, i) => (
            <div key={i} style={{
              background: "#f6f7fb",
              borderRadius: "13px",
              ...
              <div style={{ flex: 2 }}>
                ...
                <div style={{ fontSize: "1rem", marginTop: "7px" }}>
                  ...
                  <span style={{ color: "#6366f1", marginLeft: "4px" }}>Semester:</span>
                  <span style={{ marginLeft: "7px", color: "#253059", fontWeight: 600 }}>{a.sem ?? 'N/A'}</span>
                  <span style={{ marginLeft: "24px", opacity: .72 }}><FaRegCheckCircle size={15} color={"#e74c3c"} style={{ marginBottom: "-3px" }} /> {a.grade}</span>
                </div>
              </div>
              <button
                style={{
                ...
                onClick={() => window.alert(`Start preparation for ${a.name}`)}
              >
                Start Preparation
              </button>
            </div>
